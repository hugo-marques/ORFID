...
If the a minimum time gap (in seconds) is specified using the gap argument, only detection periods where the time gap between detections was greater than the specified minimum are retained in the summary table.

```{r marker_gap, eval = TRUE}
marker_tag(dat_multi, "0000_000000005972", gap = 600)
```

In the marker tag plot, gaps between detections greater than the specified minimum are highlighted in red.

```{r marker2, eval = TRUE, fig.width = 6, fig.heigh = 5}
marker_tag_plot(dat_multi, "0000_000000005972", gap = 600)
```
...
```{r export, eval = FALSE}
export_ORFID(x = dat_multi, name = "multi_data_compiled", extension = ".xlsx")
```
